...
# Setup Flatpak and GNOME Software
setup_flatpak() {
    log "Setting up Flatpak and GNOME Software..."
    sudo apt remove -y snap-store 2>/dev/null || true
...
display_maintenance_menu() {
    ...
    echo -e "${YELLOW}5.${NC}  Update Flatpak Packages"
    echo -e "${YELLOW}6.${NC}  Install Flatpak"
    echo -e "${YELLOW}7.${NC}  Setup Flatpak & GNOME Software"
    echo -e "${YELLOW}8.${NC}  Full System Cleanup"
    echo -e "${YELLOW}0.${NC}  Back to main menu"
    echo ""
}
...
install_terraform() {
    ...
    flatpak install -y flathub org.terraform.Terraform 2>/dev/null || {
        warning "Terraform not available on Flatpak, installing via APT..."
...
install_chrome() {
    ...
    flatpak install -y flathub com.google.Chrome 2>/dev/null || {
        warning "Chrome not available on Flatpak, installing via DEB..."
...
# Initial setup - Remove snap-store and install flatpak/gnome-software
initial_setup() {
    log "Verificando e configurando sistema inicial..."
    
    # Remove snap-store completamente
    if dpkg -l | grep -q snap-store; then
        log "Removendo snap-store..."
        sudo apt remove --purge -y snapd-desktop-integration snap-store
        sudo apt autoremove -y
    fi
    ...
    # Verifica se gnome-software está instalado
    if ! dpkg -l | grep -q gnome-software; then
        log "GNOME Software não encontrado. Instalando..."
        sudo apt install -y gnome-software gnome-software-plugin-flatpak
    fi
    
    # Adiciona repositório Flathub se não existir
